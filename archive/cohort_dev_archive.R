...
#' @export
define_criteria <- function(codeset = NULL, start_date, end_date, n_code_req, days_sep) {
  # Validate inputs
	if (!is.null(codeset)) {
  if (!is.tbl(codeset)) {
...
#' @export
define_criteria.diagnosis <- function(codeset, start_date, end_date, n_code_req, days_sep) {
  ...
  message("Processing diagnosis table...")
	...
	distinct_visit_requirement_pats <-
		...
		distinct(patid, first_coalesced_dx_admit)
...
#' @export
define_criteria.procedure <- function(codeset, start_date, end_date, n_code_req, days_sep) {
	# Implementation for diagnosis codes
	message("Processing diagnosis table...")
	...
	distinct_visit_requirement_pats <-
		...
		filter(days_sep_from_first >= days_sep &
					 	days_sep_from_last >= days_sep) %>%
		distinct(patid, first_coalesced_px_admit)
...
#' @export
define_criteria.default <- function(codeset, start_date, end_date, n_code, n_days) {
	cli_abort(
