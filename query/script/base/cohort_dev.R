...
match_codetype_to_table <- function() {
  tibble::tribble(~codetype, ~table, ~pcornet_vocab_type,
    ...
    copy_to_new(df = ., name = "crosswalks", overwrite = TRUE)
}

get_table_config <- function(codeset) {
  codetype_mapping <- codesets$crosswalk %>%
...
define_criteria.generic <- function(cohort = NULL, codeset, start_date, end_date,
  ...
  if(.GlobalEnv$cdm_type == "pcornet") {
    ...
    if(all(pull(codeset, code) == "*")) {
      ...
      type_value <- codesets$crosswalk %>% filter(codetype %in% codetype_value) %>% pull()

      cohort_with_codes <- cdm_tbl(table_name) %>%
