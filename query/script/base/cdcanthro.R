...
# SOURCE ATTRIBUTION
# -------------------------------------------------------------------------
# Original Script Name: cdcanthro.R
# Source Repository:    https://github.com/CDC-DNPAO/CDCAnthro
# Original Author:      David Freedman (https://github.com/CDC-DNPAO)
# Retrieval Date:       11/19/2025
#
# DESCRIPTION:
# This script was downloaded from the https://github.com/CDC-DNPAO to implement
# GENERATE SEX- AND AGE-STANDARDIZED WEIGHT, HEIGHT,
# AND BMI METRICS FROM THE CDC GROWTH CHARTS (non-obese children)
# AND FROM THE ‘EXTENDED METHOD’ (children with obesity).
#
# -------------------------------------------------------------------------
...
grepv <- function(..., ignore.case = T, value = T) {
  base::grep(..., ignore.case = T, value = T)
}
set_cols_first <- function(DT, cols, intersection = TRUE) {
  ...
  if (intersection) {
    return(setcolorder(
      DT,
      c(intersect(cols, names(DT)), setdiff(names(DT), cols))
    ))
  } else {
...
cdcanthro <- function(
  ...
  age_in_months <- weight <- height <- seq_ <- sex <- agey <- bz <-
    ...
    hap <- wap <- orig_bmiz <- orig_bmip <- perc_median <- NULL
  ...
  if (length(nms) != 1) {
    stop(
      "A child's sex MUST be named 'sex'; this is case insensitive.
  ...
  data <- data[
    between(age, 24, 239.9999), #  & !(is.na(wt) & is.na(ht)),
  ...
  setnames(dt, cc(bz, bmip, ebp, ebz), cc(orig_bmiz, orig_bmip, bmip, bmiz))
...
load_cdc_refs <- function() {
  e <- new.env()
  load("./query/script/base/cdc__ref__data.rda", envir = e)
  
  e$cdc__ref__data %>% as_tibble()
  
  .GlobalEnv$cdc__ref__data <- e$cdc__ref__data %>% as.data.table()
}
