...
initialize_session <- function(
  ...
  get_argos_default()$config("subdirs", if (packageVersion("argos") >= '0.3') {
    list(specs = specs_drop_wd, results = results_drop_wd)
  } else {
...
pkgLoad <- function() {
  ...
  # Define required package versions (allows multiple versions using a vector)
  required_versions <- list(
    "DBI" = "1.2.3",
    "odbc" = c("1.7.0", "1.5.0"), # The first element ("1.7.0") is the default install if neither is found
    "ggplot" = "4.0.0",
    "dplyr" = c("1.1.4","1.2.1"),
    "dbplyr" = "2.5.0",
    "tidyverse" = "2.0.0"
  ...
  # Function to check if package version matches ANY of the requirements
  check_version <- function(pkg_name, required_version) {
    ...
    if (pkg_name %in% installed_pkgs[, "Package"]) {
      ...
      # Check if installed version matches any in the allowed array
      matches <- sapply(required_version, function(ver) {
        utils::compareVersion(installed_version, ver) == 0
      })
      return(any(matches))
    }
    return(FALSE)
    ...
    sapply(package_list, function(pkg) {
      if (pkg %in% names(required_versions)) {
        ...
        if (check_version(pkg, required_versions[[pkg]])) {
          return(1) # Package exists with a correct version
        } else {
    ...
    for (source in names(to_install)) {
      if (source == "CRAN") {
        for (pkg in to_install[[source]]) {
          if (pkg %in% names(required_versions)) {
            # Install the first version specified in the list if neither is found
            target_version <- required_versions[[pkg]][1]
            version_spec <- paste0(pkg, "@", target_version)
  ...
  for (pkg in names(required_versions)) {
    if (pkg %in% loadedNamespaces()) {
      ...
      status <- if (any(matches)) "✓" else "✗"
      req_str <- paste(required_version, collapse = " or ")
