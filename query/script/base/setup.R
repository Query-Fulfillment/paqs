...
patch_argos <- function() {
  ...
  argos$public_methods$`copy_to_new.Spark SQL` <- function(
    ...
    schema_qualfied_name <- DBI::Id(schema = self$config('temp_table_schema'), table = name)
    rslt <- batch_write_databricks(data = df,conn = dest, table_id = schema_qualfied_name)
...
#' @export
batch_write_databricks <- function(data, 
                                   conn, 
                                   table_id, 
  ...
  cli::cli_alert_info("Total rows: {.val {n_rows}}, Batch size: {.val {batch_size}}, Batches: {.val {n_batches}}")
  ...
  for (i in 1:n_batches) {
    ...
    tryCatch({
      ...
      cli::cli_alert_danger("Error writing batch {.val {i}} (rows {start_idx}-{end_idx})")
      ...
      stop(paste0("Batch write failed at batch ", i, ". ODBC Error: ", err_msg), call. = FALSE)
    })
  }
  
  cli::cli_progress_done()
  cli::cli_alert_success("Successfully wrote {.val {n_rows}} rows to {.field {table_id_log}}")
